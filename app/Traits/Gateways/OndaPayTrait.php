...
trait OndaPayTrait
{
    ...
    private static function generateCredentialsOnda()
    {
        ...
        if (!empty($setting)) {
            ...
            self::$uriOnda = rtrim($setting->getAttributes()['ondapay_uri'], '/');
    ...
    public function requestQrcodeOnda($request)
    {
        try {
            ...
            // Preparar dados para a API Divpag
            $postData = [
            ...
            $url = self::$uriOnda . '/pix/qrcode';

            Log::info('[DIVPAG] Requisição QRCode', [
                'url' => $url,
                'data' => $postData
            ]);
            ...
            Log::info('[DIVPAG] Resposta QRCode', [
            ...
            if ($response->successful() && is_array($responseData)) {
                if (isset($responseData['qrcode']) && isset($responseData['transactionId'])) {
                    self::generateTransactionOnda(
                        ...
                        $responseData['external_id'] ?? $idUnico
                    );
                    ...
                    Log::error('[DIVPAG] Resposta sem qrcode ou transactionId', [
            ...
            Log::error('[DIVPAG] Erro na requisição', [
                'status' => $response->status(),
                'body' => $response->body()
            ]);
            ...
            Log::error('[DIVPAG] Exception no QRCode', [
    ...
    {
        try {
            Log::info('[DIVPAG] Webhook recebido', $request->all());
            ...
            if ($transactionType === 'RECEIVEPIX') {
                ...
                if ($status === 'PAID') {
                    if (self::finalizaPaymentOnda($transactionId, $external_id)) {
                ...
                if (isset($statusCode['statusId']) && $statusCode['statusId'] == 1) {
                    Log::info('[DIVPAG] Saque confirmado', $request->all());
            ...
            Log::error('[DIVPAG] Erro no webhook', [
                'message' => $e->getMessage(),
                'line' => $e->getLine()
            ]);
    ...
    public function pixCashOutOnda($id, $tipo)
    {
        ...
        if ($withdrawal != null) {
            ...
            // Monta a URL completa
            $url = self::$uriOnda . '/pix/payment';

            Log::info('[DIVPAG] Payload de saque', [
                'url' => $url,
                'data' => $postData
            ]);
            ...
            Log::info('[DIVPAG] Resposta de saque', [
                'status' => $response->status(),
                'body' => $response->body(),
                'json' => $responseData
            ]);
            ...
            if ($response->successful() && is_array($responseData)) {
                // CORREÇÃO: Verifica statusCode direto no objeto, não em array[0]
                if (isset($responseData['statusCode']) && $responseData['statusCode'] == 200) {
                    ...
                    Log::info('[DIVPAG] Saque aprovado e atualizado no banco', [
                        'withdrawal_id' => $withdrawal->id,
                    ...
                    Log::error('[DIVPAG] Erro no saque', [
            ...
            Log::error('[DIVPAG] Falha na requisição de saque', [
                'status' => $response->status(),
                'body' => $response->body()
            ]);
